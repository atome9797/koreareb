...
public class ViewChangeHistory : MonoBehaviour
{
    public string MenuName = "변경 이력";

    public MainManager mainManager;

    public ServerManager serverManager;

    public Item_ChangeLog item_change_log;
    ...
    //사용자 정보
    List<Item_ChangeLog> list_change_log;//단지 스크립트

    bool[] _dropdownType = new bool[3];//검색구분
    ...
    public string[] colname = { "NO","성명","메뉴명","변경사항", "변경일시" };

    void Awake()
    {
        list_change_log = new List<Item_ChangeLog>(); //사용자 정보
    ...
    public void SaveDataTableToCsv(DataTable dataTable, string filePath, string [] columname)
    {
        ...
        using (StreamWriter streamWriter = new StreamWriter(filePath, false, Encoding.GetEncoding("EUC-KR")))
        {
            ...
            foreach (DataRow row in dataTable.Rows)
            {
                ...
                values.Add(row["change_name"].ToString());
                values.Add(row["change_menuname"].ToString());
                values.Add(row["change_contents"].ToString());
    ...
    public void Action_Search()
    {
        ...
        CheckTable = serverManager.GetLog("tb_change_log", SelectMenu.dateText.FromDate, SelectMenu.dateText.ToDate, _dropdownType, SelectMenu.input_search.text);
    ...
    {
        var children = contents.transform.GetComponentsInChildren<Item_ChangeLog>(true);
    ...
    public void UpdateView()
    {
        ...
        for (var rowIndex = startIndex; rowIndex < endIndex; rowIndex++)
        {
            ...
            Item_ChangeLog item = Instantiate(item_change_log, contents);

            item.no.text = row["NO"].ToString();
            item.user_name.text = row["change_name"].ToString();
            item.menu_name.text = row["change_menuname"].ToString();
            item.contents.text = row["change_contents"].ToString();
            item.change_date.text = row["날짜"].ToString();

            list_change_log.Add(item);
        }

        if (list_change_log.Count == 0)
        {
            item_Empty.SetActive(true);
