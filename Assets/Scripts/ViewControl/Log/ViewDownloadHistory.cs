...
public class ViewDownloadHistory : MonoBehaviour
{
    public string MenuName = "다운로드 이력";

    public MainManager mainManager;

    public ServerManager serverManager;

    public Item_DownLog item_down_log;
    ...
    //사용자 정보
    List<Item_DownLog> list_Down_log;//단지 스크립트
    ...
    public string[] colname = { "NO","성명","메뉴명","다운로드 이력","다운로드 일시" };

    void Awake()
    {
        list_Down_log = new List<Item_DownLog>(); //사용자 정보
    ...
    public void SaveDataTableToCsv(DataTable dataTable, string filePath, string [] columname)
    {
        ...
        using (StreamWriter streamWriter = new StreamWriter(filePath, false, Encoding.GetEncoding("EUC-KR")))
        {
            ...
            foreach (DataRow row in dataTable.Rows)
            {
                ...
                values.Add(row["down_name"].ToString());
                values.Add(row["down_menu"].ToString());
                values.Add(row["down_contents"].ToString());
    ...
    public void Action_Search()
    {
        ...
        CheckTable = serverManager.GetLog("tb_down_log", SelectMenu.dateText.FromDate, SelectMenu.dateText.ToDate, _dropdownType, SelectMenu.input_search.text);
    ...
    {
        var children = contents.transform.GetComponentsInChildren<Item_DownLog>(true);
    ...
    public void UpdateView()
    {
        ...
        for (var rowIndex = startIndex; rowIndex < endIndex; rowIndex++)
        {
            ...
            Item_DownLog item = Instantiate(item_down_log, contents);

            item.no.text = row["NO"].ToString();
            item.user_name.text = row["down_name"].ToString();
            item.menu_name.text = row["down_menu"].ToString();
            item.contents.text = row["down_contents"].ToString();
            item.change_date.text = row["날짜"].ToString();

            list_Down_log.Add(item);
        }

        if (list_Down_log.Count == 0)
        {
            item_Empty.SetActive(true);
